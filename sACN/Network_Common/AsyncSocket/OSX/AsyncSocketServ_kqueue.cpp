// AsyncSocketServ_kqueue.cpp: implementation of the CAsyncSocketServ class.
//
//This class implements the OSX-Specific IAsyncSocketServ implementation
//of socket support that uses blocking sockets, with a thread that uses kqueue
//to determine when sockets are ready for reading.
//
// NOTE: At the time of writing this, kqueue seemed to be more inefficient in the bulktest than
// a straight select() implementation.  Therefore, this code is not currently used.
// Also, a search on-line showed that OSX 10.4 had problems in its implementation of kqueue, so
// since we need to support that version we'll keep using select.
//
//////////////////////////////////////////////////////////////////////
...
#include "AsyncSocketServ_kqueue.h"
...
//Only one instance of this thread does the kqueue, and does a
...
void* AsyncSocket_ReadThread(void* p)
{
	...
	//StartAutoRecv and StopAutoRecv handle tweaking the internal kevent map, we just need to query here.
	
	//kevent() requires an array of kevent structures to put the results in.  We'll have a standing
	//list loosly based on the current number of sockets, plus some slop.  To also give slop, we'll
	//count the disconnected and manual sockets as well.  This will grow, unless we are some largeish
	//number over the current number.
	struct kevent* klist = NULL;
	int ksize = 0;
	
	// Each cycle of the main loop is a blocking kevent
	// Check for termination each time.
	while(!psrv->m_terminated)
	{
		int socklist_size = 0;  //Get a general read on the socket map size --  See the earlier comment
		
		//TESTING -- since we just want a snapshot of the size, which is a straight int under the covers, do we need to lock?
		//if(psrv->m_socklock.ReadLock())
		{
			socklist_size = psrv->m_sockmap.size() + 5;  //give it some slop.
		//	psrv->m_socklock.ReadUnlock();
		}
		
		//tweak klist
		if((ksize < socklist_size) || (ksize - socklist_size > 50))
		{
			if(klist)
				delete [] klist;
			ksize = socklist_size;
			klist = new struct kevent [ksize];
		...
		int event_res = kevent(psrv->m_kq_id, NULL, 0, klist, ksize, &tv); 
		
		if((event_res > 0) && !psrv->m_terminated && psrv->m_socklock.ReadLock())
		{
			for(int i = 0; (i < event_res) && !psrv->m_terminated ; ++i)
				psrv->SocketRead((CAsyncSocketServ::socketref*)(klist[i].udata));  //Because StopAutoReceive removes the kevent before closing the socket, udata should always be valid.
...
void CAsyncSocketServ::DestroySocket(sockid id)
{
	if ( m_socklock.WriteLock() )
    {
    	...
    	{
            //Kill the socket
            SocketStopAutoRecv(&sockit->second);
...
bool CAsyncSocketServ::SocketStartAutoRecv(socketref* pref)
{
	if(!pref->m_ismanual)
	{
		//Add a kevent to the queue
		struct kevent k;
		EV_SET(&k, pref->m_Socket, EVFILT_READ, EV_ADD, 0, 0, pref);
		
		if(kevent(m_kq_id, &k, 1, NULL, 0, 0) < 0)
			return false;
...
void CAsyncSocketServ::SocketStopAutoRecv(socketref* pref)
{
	if(!pref->m_ismanual)
	{
		//Remove the kevent from the queue
		struct kevent k;
		EV_SET(&k, pref->m_Socket, EVFILT_READ, EV_DELETE, 0, 0, pref);
		
		kevent(m_kq_id, &k, 1, NULL, 0, 0);
	}
}
