...
static int get_ifaces(iface_t **if_head)
{
  ...
  for (pAdapter = pAdapterInfo; pAdapter && pAdapter < pAdapterInfo + ulOutBufLen; pAdapter = pAdapter->Next)
  {
    if (pAdapter->Type != MIB_IF_TYPE_ETHERNET && pAdapter->Type != IF_TYPE_IEEE80211)
      continue;

    for (ipAddress = &pAdapter->IpAddressList; ipAddress; ipAddress = ipAddress->Next)
    {
 ...
 */
static void add_iface_if_needed(iface_t **head, iface_t **tail, struct ifaddrs *ifa)
{
...
int artnet_net_recv(node n, artnet_packet p, int delay)
{
  ...
  len = recvfrom(n->sd,
                 ...
                 sizeof(p->data), 0, (SA *)&cliAddr, &cliLen);
  if (len < 0)
  {
...
int artnet_net_send(node n, artnet_packet p)
{
  ...
  ret = sendto(n->sd,
               ...
               p->length, 0, (SA *)&addr, sizeof(addr));
  if (ret == -1)
  {
...
int artnet_net_inet_aton(const char *ip_address, struct in_addr *address)
{
  ...
  if ((*addr = inet_addr(ip_address)) == INADDR_NONE && strcmp(ip_address, "255.255.255.255"))
  {
#endif
